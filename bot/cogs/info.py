...
class Info(commands.Cog):
    ...
    async def memberinfo(self, ctx, member : discord.Member):
        ...
        embed.add_field(name=f"Joined {ctx.guild.name} at:", value=member.joined_at)
        await ctx.send(embed=embed, mention_author=False)


    @commands.command(description="'server' can be the name, or id of a server GamerBot is in or left blank to get information of the current server", help="Gets informatino of a server", aliases=["serverinfo"], enabled=False, hidden=True)
