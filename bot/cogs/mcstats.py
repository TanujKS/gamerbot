...
class MinecraftStats(commands.Cog, name="Minecraft Statistics", description="Commands for Minecraft player statistics"):
    ...
    async def minecraft(self, ctx, *player):
        ...
        embed = discord.Embed(title=f"{info.name}'s Minecraft Profile", description=f"Stats for {info.name}", color=constants.Color.red())
    ...
    async def skin(self, ctx, *player):
        ...
        if not uuid:
            raise commands.BadArgument(f'Player "{player}" not found.')
        info = MojangAPI.get_profile(uuid)
        embed=discord.Embed(title=f"{info.name}'s Skin", description=f"Full render of {info.name}'s skin", color=constants.Color.red())
    ...
    async def hypixel(self, ctx, *player):
        ...
        embed = discord.Embed(title=f"{data['player']['displayname']}'s Hypixel Profile", description=f"Hypixel stats for {data['player']['displayname']}", color=constants.Color.red())
    ...
    async def hypixelguild(self, ctx, *player_or_guild):
        ...
        embed = discord.Embed(title=f"{guild['guild']['name']}'s Guild Profile", description=f"Guild stats for {guild['guild']['name']}", color=constants.Color.red())
        if member:
            embed.set_thumbnail(url=f"https://crafatar.com/renders/head/{uuid}?overlay&?{round(time.time())}")
    ...
    async def bedwars(self, ctx, *player_and_mode):
        ...
        if len(player_and_mode) < 2:
            embed=discord.Embed(title=f"{rawData['player']['displayname']}'s Hypixel Bedwars Profile", description=f"Bedwars stats for {rawData['player']['displayname']}", color=constants.Color.red())
            ...
            embed = discord.Embed(title=f"{rawData['player']['displayname']}'s Hypixel {player_and_mode[1].capitalize()} Bedwars Profile", description=f"{player_and_mode[1].capitalize()} Bedwars stats for {rawData['player']['displayname']}", color=constants.Color.red())
    ...
    async def skywars(self, ctx, *player_and_mode):
        ...
        if len(player_and_mode) <= 1:
            embed = discord.Embed(title=f"{rawData['player']['displayname']}'s Hypixel Skywars Profile", description=f"Skywars stats for {rawData['player']['displayname']}", color=constants.Color.red())
            embed.add_field(name="Coins:", value=data.get('coins', 0), inline=True)
            ...
            if joinedmode == "solos normal":
                embed = discord.Embed(title=f"{rawData['player']['displayname']}'s Hypixel Solos Normal Skywars Profile", description=f"Solo Normal Skywars stats for {rawData['player']['displayname']}", color=constants.Color.red())
                embed.add_field(name="EXP:", value=data.get('skywars_experience', 0), inline=True)
                embed.add_field(name="Level:", value=getSkyWarsLevel(data.get('skywars_experience', 0)), inline=True)
                ...
                embed = discord.Embed(title=f"{rawData['player']['displayname']}'s Hypixel Solos Insane Skywars Profile", description=f"Solo Insane Skywars stats for {rawData['player']['displayname']}", color=constants.Color.red())
                embed.add_field(name="EXP:", value=data.get('skywars_experience', 0), inline=True)
                embed.add_field(name="Level:", value=getSkyWarsLevel(data.get('skywars_experience', 0)), inline=True)
                ...
                embed = discord.Embed(title=f"{rawData['player']['displayname']}'s Hypixel Teams Normal Skywars Profile", description=f"Teams Normal Skywars stats for {rawData['player']['displayname']}", color=constants.Color.red())
                embed.add_field(name="EXP:", value=data.get("skywars_experience", 0), inline=True)
                embed.add_field(name="Level:", value=getSkyWarsLevel(data.get("skywars_experience", 0)), inline=True)
                ...
                embed = discord.Embed(title=f"{rawData['player']['displayname']}'s Hypixel Teams Insane Skywars Profile", description=f"Teams Insane Skywars stats for {rawData['player']['displayname']}", color=constants.Color.red())
                embed.add_field(name="EXP:", value=data.get('skywars_experience', 0), inline=True)
                embed.add_field(name="Level:", value=getSkyWarsLevel(data.get("skywars_experience", 0)), inline=True)
    ...
    async def duels(self, ctx, *player_and_mode):
        ...
        if len(player_and_mode) < 2:
            embed = discord.Embed(title=f"{rawData['player']['displayname']}'s Hypixel Duels Profile", description=f"Duels stats for {rawData['player']['displayname']}", color=constants.Color.red())
            mode = "all_modes"
            for ra in ranks:
            ...
            embed = discord.Embed(title=f"{rawData['player']['displayname']}'s Hypixel {mode.capitalize()} Duel Profile", description=f"{mode.capitalize()} duel stats for {rawData['player']['displayname']}", color=constants.Color.red())
            for ra in ranks:
