...
class Stats(commands.Cog, description="Commands for player statistics for all supported games"):
                    ...
                    if channel['id'] == str(trackingGuilds[guild][index]['id']):
                        ...
                        if is_live:
                            if trackingGuilds[guild][index]['pinged'] == "False":
                                embed = discord.Embed(title=trackingGuilds[guild][index]['message'], description=f"https://twitch.tv/{trackingGuilds[guild][index]['streamer']}", color=embedColors["Red"])
    ...
    async def twitchProfile(self, channel, data):
        embed = discord.Embed(title=f"{data['display_name']}'s' Twitch Stats", description=f"https://twitch.tv/{channel}", color=embedColors["Red"])
    ...
    async def twitchtrack(self, ctx, channel, message=None):
        ...
        embed = discord.Embed(title=f"THIS IS AN EXAMPLE STREAM: {trackingGuilds[ctx.guild.id][-1]['message']}", description=f"https://twitch.tv/{trackingGuilds[ctx.guild.id][-1]['streamer']}", color=embedColors["Red"])
    ...
    async def twitchtracklist(self, ctx):
        trackingGuilds = utils.loadTrackingGuilds(r)
        embed = discord.Embed(title=f"All Twitch Tracks in {ctx.guild.name}", color=embedColors["Red"])
        for track in trackingGuilds[ctx.guild.id]:
    ...
    async def youtube(self, ctx, *, channel):
        ...
        for item in data['items']:
            if item != data['items'][0]:
                embed = discord.Embed(title=f"YouTube statistics for {item['snippet']['title']}", description=f"https://www.youtube.com/channel/{item['snippet']['channelId']}", color=embedColors["Red"])
    ...
    async def csgo(self, ctx, *player):
        ...
        if not data:
            raise commands.BadArgument(rawData['errors'][0]['message'])
        embed = discord.Embed(title=f"{data['platformInfo']['platformUserHandle']}'s CS:GO Profile", description=f"Stats for {data['platformInfo']['platformUserHandle']}", color=embedColors["Red"])
