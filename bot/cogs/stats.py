...
class Stats(commands.Cog, description="Commands for player statistics for all supported games"):
    ...
    async def youtube(self, ctx, *, channel):
        ...
        for channel in data['items']:
            ...
            embed.add_field(name="Views:", value=utils.insert_commas(stats.get('viewCount', 0)), inline=True)
            embed.add_field(name="Subscribers:", value=utils.insert_commas(stats.get('subscriberCount', 0)), inline=True)
            embed.add_field(name="Videos:", value=utils.insert_commas(stats.get('videoCount')), inline=True)
            embed.set_thumbnail(url=channel['snippet']['thumbnails']['default']['url'])
            embed.set_footer(text=f"Stats provided by the YouTube API \nPage {data['items'].index(channel)+1}")
